...
class TextEditorComponent
  ...
  screenPositionForPixelPosition: (pixelPosition) ->
    row = @linesYardstick.rowForPixelPosition(pixelPosition)
    unless @presenter.isRowVisible(row)
      @presenter.setScreenRowsToMeasure([row])
      @updateSyncPreMeasurement()

    column = @linesYardstick.columnForRowAndPixelPosition(row, pixelPosition)
    @presenter.clearScreenRowsToMeasure()
    Point(row, column)
